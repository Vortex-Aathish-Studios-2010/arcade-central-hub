...
export const WordSearchGame = ({ level: propLevel, onComplete }: Props) => {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-2">
      ...
      <p className="text-[10px] text-muted-foreground">Click letters in a line or drag to select</p>

      <div ref={gridRef} className="select-none" onMouseUp={handleDragEnd} onMouseLeave={handleDragEnd}
            ...
            {row.map((letter, c) => (
              <div
                ...
                className={`${cellSize} flex items-center justify-center font-display font-bold cursor-pointer transition-all rounded-sm
                  ${foundCells.has(cellKey(r, c)) ? "bg-primary/30 text-primary" : selectingSet.has(cellKey(r, c)) ? "bg-accent/30 text-accent" : "text-foreground/80 hover:bg-muted/50"}
                `}
              >
                {letter}
              </div>
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {config.words.map((word) => (
          <span key={word} className={`text-xs font-display px-2 py-0.5 rounded ${found.has(word) ? "bg-primary/20 text-primary line-through" : "bg-card text-muted-foreground border border-border"}`}>
            {word}
          </span>
        ))}
      </div>
      ...
      {selecting.length > 0 && !isDragging && (
        <div className="flex items-center gap-2">
          <span className="text-xs font-display text-accent">{selecting.map(([r, c]) => grid[r][c]).join("")}</span>
          <button onClick={() => setSelecting([])} className="px-3 py-1.5 bg-card border border-border text-foreground rounded-lg font-display text-xs hover:border-destructive/50 transition-all">
            CLEAR
          </button>
        </div>
      )}

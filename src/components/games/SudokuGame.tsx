...
export const SudokuGame = ({ level: propLevel, onComplete }: Props) => {
  ...
  const [board, setBoard] = useState(() => initialPuzzle.map((r) => [...r]));
  const [fixed] = useState(() => initialPuzzle.map((r) => r.map((v) => v !== 0)));
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4">
            ...
            {row.map((val, c) => (
              <motion.div
                ...
                className={`w-8 h-8 flex items-center justify-center text-sm font-display cursor-pointer transition-colors
                  ...
                  ${selected && selected[0] === r && selected[1] === c ? "bg-primary/20" : ""}
      ...
      {!won && !showingSolution && (
        <button onClick={handleViewSolution} className="px-4 py-2 bg-secondary/10 border border-secondary/30 text-secondary rounded-lg font-display text-xs hover:bg-secondary/20 transition-all flex items-center gap-1.5">
      ...
      {won && (
        <button onClick={() => window.location.reload()} className="px-6 py-2 bg-primary text-primary-foreground rounded-xl font-display text-sm">
          NEXT LEVEL →
        </button>
      )}
    </div>

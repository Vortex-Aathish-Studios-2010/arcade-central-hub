...
export const createRoom = async (playerId: string, gameId: string): Promise<{ id: string; code: string }> => {
  ...
  const { data, error } = await supabase.from("game_rooms").insert({ code, game_id: gameId, host_id: playerId, status: "waiting" }).select("id, code").single();
...
export const joinRoom = async (playerId: string, code: string): Promise<{ id: string; game_id: string } | null> => {
  const { data: room } = await supabase.from("game_rooms").select("id, game_id, status").eq("code", code.toUpperCase()).single();
...
export const reportScore = async (roomId: string, playerId: string, score: number) => {
  await supabase.from("room_players").update({ score }).eq("room_id", roomId).eq("player_id", playerId);
  await supabase.from("game_rooms").update({ status: "finished" }).eq("id", roomId);
};

export const subscribeToRoom = (roomId: string, callback: (players: any[], status: string) => void) => {
  const channel = supabase.channel(`room-${roomId}`)
    .on("postgres_changes", { event: "*", schema: "public", table: "room_players", filter: `room_id=eq.${roomId}` }, async () => {
      ...
      callback(ps || [], room?.status || "waiting");
    })
